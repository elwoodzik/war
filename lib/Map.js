...
class Map {
        ...
        mapTilesLayers.forEach((map, index) => {
            ...
            for (let i = 0; i < map.tilesLayer.length; i++) {
                ...
                for (let j = 0; j < map.tilesLayer[i].length; j++) {
                    ...
                    // ctx.fillRect(
                    //     map.tilesLayer[i][j].x,
                    //     map.tilesLayer[i][j].y,
                    //     32,
                    //     32
                    // );
                    ...
                    // for (var i = 0; i < this.fogArray.length; i++) {
    ...
    addToFog(x, y, tileWidth) {

        this.fogArray.push({ x: x, y: y })
        // this.fogArray.push(new Fog(this.game, {
        //     x: x + this.game.camera.xScroll,
        //     y: y,
        //     fill: 'black',
    ...
    getTileBySprite(sprite, disableMargin) {
        const mapData = this.getMapData();
        const center = sprite.getCenter();
        let margin = 0;
        // + sprite.height - 32
        if (disableMargin && sprite.objectType === 'unit') {
            margin = sprite.height - 32
        }
        const row = Math.floor(center.x / mapData.tilewidth);// Math.round(x / this.pathfinderJson.width - y / this.pathfinderJson.height);//
        const column = Math.floor((center.y + margin) / mapData.tileheight);
