...
class PathMove {
    ...
    findPathCallback(newPath) {
        ...
        if (this.path.length > 0) {
            ...
            if (this.extendsMove && typeof this.extendsMove === 'function') {
                ...
                if (bool === 'stop') {
                    ...
                    if (this.waitToPathWillBeFree > 0) {
                        ...
                        this.timeOut = setTimeout(this.move.bind(this), this.timeOutSpeed, this.endPos ? this.endPos : null, this.moveCallback, this.follow)
                        return false;
                    } else {
            ...
            if (this.nextTile.type === 'solid' || this.nextTile.type === 'town') {
                this.sprite.isMoving = false;
                // if (this.spriteAnimation && typeof this.spriteAnimation === 'function') {
                //     this.spriteAnimation(this.currentTile, this.nextStep);
                // }
                ...
                if (this.path.length > 0) {
                    if (this.waitToPathWillBeFree > 0) {
                        this.waitToPathWillBeFree--;
                        //  this.move(this.endPos ? this.endPos : null, this.moveCallback, this.follow);
                        this.timeOut = setTimeout(this.move.bind(this), this.timeOutSpeed, this.endPos ? this.endPos : null, this.moveCallback, this.follow)
                        return false;
                    } else {
                        console.log('nie udało się wyszukać dostepnęj scieżki!')
                    ...
                    if (this.waitToPathWillBeFree > 0) {
                        ...
                        if (this.moveCallback && typeof this.moveCallback === 'function') {
                            ...
                            console.log('tutaj 222', this.sprite.objID)
                            if (this.spriteAnimation && typeof this.spriteAnimation === 'function') {
                                this.spriteAnimation(this.currentTile, this.nextStep);
                            }
                            this.sprite.attackTarget = false;
            ...
            console.log('o tutaj to zachodzi', this.sprite.objID)
            return false;
        }
    }
    ...
    followEnemy = (enemy) => {
        ...
        this.move(null, (path, player) => {
            ...
            if (enemy.used && enemy.isMoving && this.path.length > 0) {
    ...
    followBuilding = (enemy) => {
        this.sprite.attackTarget = true;
        const endPos = this.findShortPathToBuilding(enemy, 2);

        this.move(endPos, (path, player) => {
            player.isAttacking = false

            if (enemy.isMoving && this.path.length > 0) {
                this.followBuilding(enemy);
            } else {
                player.isAttacking = true;
                if (this.spriteAnimation && typeof this.spriteAnimation === 'function') {
                    this.spriteAnimation(this.currentTile, this.nextStep, this.onHitBuilding.bind(this, enemy));
                }

                player.animations.play({
                    key: player.dir,
                    callback: this.onHitBuilding.bind(this, enemy)
                })
            }
        });
    }
    ...
    onHitEnemy(enemy) {
        if (enemy.used) {
            ...
            if (enemy.currentHp <= 0) {
                ...
                if (this.game.VAR.sellectedObj && enemy.objID === this.game.VAR.sellectedObj.objID) {
                    enemy.buildingPut.hide();
    ...
    onHitBuilding(enemy) {
        ...
        if (enemy.currentHp <= 0) {
            ...
            // if (enemy.pathMove.nextTile && (enemy.pathMove.nextTile.row !== this.currentTile.row || enemy.pathMove.nextTile.column !== this.currentTile.column)) {
            //     enemy.pathMove.nextTile.type = 'empty';
            //     this.game.easystar.setAdditionalPointCost(enemy.pathMove.nextTile.row, enemy.pathMove.nextTile.column, 0);
            // }
            // if (enemy.pathMove.currentTile) {
